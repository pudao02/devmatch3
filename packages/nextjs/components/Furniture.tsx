...
const Furniture: React.FC<FurnitureProps> = ({ onLoaded }) => {
  const [loadedCount, setLoadedCount] = useState(0);
  const totalFurniture = 20; // Total number of furniture pieces
  const hasCalledOnLoaded = useRef(false);

  useEffect(() => {
    // Simulate progressive furniture loading
    const timer = setTimeout(() => {
      setLoadedCount(totalFurniture);
  ...
  }, []); // Remove onLoaded from dependencies
