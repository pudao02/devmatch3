export default function ResultTab({ results }) {
  return (
    <div style={{
      ...
      <div style={{ fontWeight: 'bold', marginBottom: '14px', fontSize: '18px', letterSpacing: '1px', color: '#3a5ca8' }}>Results</div>
      {results && results.length > 0 ? (
        results.map((res, i) => (
          <div key={i} style={{ background: '#f5faff', padding: '10px', borderRadius: '10px', border: '2px solid #b3d1f7', color: '#22334d', boxShadow: '2px 3px 0 #b3d1f7' }}>
            {res}
          </div>
        ))
      ) : (
